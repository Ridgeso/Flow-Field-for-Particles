...
void setVertexBufferData(VertexBuffer* vertex, size_t count, Vertice* vertices, uint32_t drawType) {
    vertex->vertCount = count;
    GLCall(glBindBuffer(GL_ARRAY_BUFFER, vertex->renderID));
    GLCall(glBufferData(GL_ARRAY_BUFFER, vertex->vertCount*sizeof(Vertice), (void*)vertices, drawType));
}
...
void setIndexBufferData(IndexBuffer* index, size_t count, Indice* indices, uint32_t drawType) {
    index->indCount = count;
    GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index->renderID));
    GLCall(glBufferData(GL_ELEMENT_ARRAY_BUFFER, index->indCount*sizeof(Indice), (void*)indices, drawType));
}