...
static Mesh Font;
void initRenderer() {
    initMesh(&Font, DrawElements, GL_TRIANGLES, "../shaders/font.vert", "../shaders/font.frag", 0, NULL, 0, NULL, GL_DYNAMIC_DRAW, 0, "../shaders/assets/font.png");
}
...
void initMesh(
    ...
    setVertexBufferData(&mesh->vertBuff, vertCount, vertices, drawMode);
    ...
    setIndexBufferData(&mesh->indexBuff, indCount, indices, drawMode);
...
void drawMesh(Mesh* mesh, const Camera* camera) {
    ...
    case DrawArray:
        GLCall(glDrawArrays(mesh->primitive, 0, mesh->vertBuff.vertCount)); break;
    case DrawElements:
        GLCall(glDrawElements(mesh->primitive, mesh->indexBuff.indCount*3, GL_UNSIGNED_INT, NULL)); break;
    }
